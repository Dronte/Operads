...
instance (Ord a, Show a, Num n, Show n, Eq n, TreeOrdering t) => Num (OperadElement a n t) where
...
oe :: (Ord a, Show a, TreeOrdering t, Num n, Eq n) => [(OrderedTree a t, n)] -> OperadElement a n t
oe = fromList

-- | Construct a monomial in the free operad from a tree and a tree ordering. It's coefficient will be 1.
oet :: (Ord a, Show a, TreeOrdering t, Num n, Eq n) => DecoratedTree a -> OperadElement a n t
oet dect = oe $ [(OT dect ordering, 1)]

-- | Construct a monomial in the free operad from a tree, a tree ordering and a coefficient.
oek :: (Ord a, Show a, TreeOrdering t, Num n, Eq n) => DecoratedTree a -> n -> OperadElement a n t
...
zero :: (Ord a, Show a, TreeOrdering t, Num n, Eq n) => OperadElement a n t 
zero = oe [(ot $ leaf 1, 0)]

-- | Check whether an element is equal to 0. 
isZero :: (Ord a, Show a, TreeOrdering t, Num n, Eq n) => OperadElement a n t -> Bool
isZero (OE m) = Map.null $ Map.filter (/=0) m

-- | Extract the leading term of an operad element as an operad element.
leadingOTerm :: (Ord a, Show a, TreeOrdering t, Num n, Eq n) => OperadElement a n t -> OperadElement a n t
